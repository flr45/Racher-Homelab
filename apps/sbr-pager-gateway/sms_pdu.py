"""Decode SMS-DELIVER PDUs returned by common GSM modems."""
...
def decode_sms_deliver_pdu(index: int, pdu_hex: str) -> DecodedSmsPart:
    ...
    if first_octet & 0x40:
        ...
        concat_reference, concat_total, concat_part = _parse_concat_header(user_data[1:header_length])

    if alphabet == "gsm7":
        header_septets = math.ceil(header_length * 8 / 7) if header_length else 0
        text_septets = max(0, user_data_length - header_septets)
        values = unpack_septets(user_data, text_septets, bit_offset=header_septets * 7)
...
def assemble_parts(parts: list[DecodedSmsPart]) -> list[dict]:
    ...
    for part in parts:
        if part.concat_reference is None or part.concat_total is None or part.concat_part is None:
    ...
    for (_, _, total), grouped_parts in multipart.items():
        by_sequence = {part.concat_part: part for part in grouped_parts if part.concat_part is not None}
    ...
    return sorted(messages, key=lambda message: (message["timestamp"], message["indices"][0]))
...
def _decode_address(data: bytes, digits: int, address_type: int) -> str:
    if address_type & 0x70 == 0x50:
        return decode_septets(unpack_septets(data, digits))
    value = "".join(f"{byte & 0x0F:X}{(byte >> 4) & 0x0F:X}" for byte in data)[:digits].replace("F", "")
    return f"+{value}" if address_type & 0x90 == 0x90 else value
...
def _decode_timestamp(data: bytes) -> str:
    ...
    local_time = datetime(
        2000 + values[0], values[1], values[2], values[3], values[4], values[5],
...
def _parse_concat_header(header: bytes) -> tuple[int | None, int | None, int | None]:
    ...
    while position + 2 <= len(header):
        ...
        if identifier == 0x08 and length == 4:
            return (value[0] << 8) | value[1], value[2], value[3]
