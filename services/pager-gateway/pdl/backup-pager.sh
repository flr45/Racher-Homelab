...
ENV_FILE="${PDL_ENV_FILE:-/etc/racher-pager/pdl.env}"
...
for file in session-secret vapid-private.pem; do
  if [[ -f "$STATE_ROOT/$file" ]]; then
    cp -p "$STATE_ROOT/$file" "$TMP_DIR/data/$file"
  fi
done
...
if [[ -f "$ENV_FILE" ]]; then
  mkdir -p "$TMP_DIR/etc"
  cp -p "$ENV_FILE" "$TMP_DIR/etc/pdl.env"
fi
...
# Kun vores egne navngivne arkiver berøres af retention.
